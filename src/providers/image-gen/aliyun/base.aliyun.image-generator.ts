...
export abstract class BaseAliyunImageGenerator extends BaseImageGenerator {
  ...
  protected alternateApiKey?: string;
  ...
  async refresh(): Promise<void> {
    let dashscopeKey: string | null = null;
    let qwenKey: string | null = null;
    try {
      dashscopeKey = await this.configManager.get<string>("DASHSCOPE_API_KEY");
    } catch {
      dashscopeKey = null;
    }
    try {
      qwenKey = await this.configManager.get<string>("QWEN_API_KEY");
    } catch {
      qwenKey = null;
    }
    const selectedKey = dashscopeKey || qwenKey;
    if (!selectedKey) {
      throw new Error("DASHSCOPE_API_KEY environment variable is not set");
    }
    this.apiKey = selectedKey;
    if (dashscopeKey && qwenKey && dashscopeKey !== qwenKey) {
      this.alternateApiKey = qwenKey;
    } else {
      this.alternateApiKey = undefined;
    }
  }
  ...
  protected async submitTask<T extends AliTaskResponse>(
    ...
    try {
      ...
      const response = await sendRequest(this.apiKey);
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (
          this.alternateApiKey &&
          message.includes("Invalid API-key")
        ) {
          try {
            const retryResponse = await sendRequest(this.alternateApiKey);
